...
export default function JobApplicationModal({ job, onClose }: { job: Job; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-[130] flex min-w-0 items-end justify-center bg-[#071a33]/70 sm:items-center sm:p-4" role="presentation">
    <section role="dialog" aria-modal="true" aria-labelledby="application-title" dir="rtl" className="flex h-[100dvh] w-full min-w-0 max-w-2xl flex-col overflow-hidden bg-[#f3f8ff] text-[#12233f] sm:max-h-[94dvh] sm:rounded-[28px] sm:border-[5px] sm:border-white sm:shadow-2xl">
      <header className="flex min-w-0 items-center justify-between gap-2 border-b border-[#deebf8] bg-white px-3 py-3.5 sm:px-6"><div className="min-w-0"><h2 id="application-title" className="text-lg font-black text-[#10243e] sm:text-xl">التقديم على الوظيفة</h2><p className="text-xs font-bold text-[#7b8ea6]">أكمل بياناتك وأرفق سيرتك الذاتية</p></div><button type="button" disabled={saving} onClick={onClose} aria-label="إغلاق" className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-[#edf6ff] text-[#173b67]"><X /></button></header>
