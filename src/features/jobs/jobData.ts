...
export function newJobRow(job: NewJob, includeMediaColumns = true, includeDetailColumns = true) {
  ...
  const detailed = includeDetailColumns ? {
    ...
    salary_negotiable: Boolean(job.salaryNegotiable), whatsapp: job.whatsapp?.trim() || null,
    email: job.email?.trim() || null, application_deadline: job.applicationDeadline || null,
  ...
  return includeMediaColumns
    ? { ...base, ...detailed, image_urls: job.images?.filter(Boolean) ?? [], video_url: job.video?.trim() || null }
    : { ...base, ...detailed };
}
