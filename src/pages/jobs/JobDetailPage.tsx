...
export default function JobDetailPage() {
  ...
  return <article className="-mx-4 -my-8 min-h-screen bg-[#f3f8ff] px-3 pb-28 pt-3 text-[#12233f] sm:px-6 sm:pt-6" dir="rtl">
    <div className="mx-auto max-w-3xl">
    <header className="mb-4 flex items-center justify-between gap-3 rounded-[20px] bg-white px-2 py-2 shadow-[0_8px_24px_rgba(31,83,142,0.08)]"><Link to="/jobs" className="flex h-11 w-11 items-center justify-center rounded-xl text-[#173b67] hover:bg-[#edf6ff]" aria-label="رجوع"><ArrowRight /></Link><h1 className="text-lg font-black text-[#102642] sm:text-xl">تفاصيل الوظيفة</h1><button type="button" onClick={() => void share()} className="flex h-11 w-11 items-center justify-center rounded-xl text-[#173b67] hover:bg-[#edf6ff]" aria-label="مشاركة الوظيفة"><Share2 className="h-5 w-5" /></button></header>
    ...
    <div>
      ...
      <div className="space-y-5 py-5 sm:py-7">
        <section className="rounded-[22px] bg-white p-5 shadow-[0_8px_26px_rgba(31,83,142,0.08)]"><div className="flex items-start justify-between gap-3"><div><h2 className="text-2xl font-black leading-tight text-[#10243e] sm:text-3xl">{job.title}</h2><p className="mt-1 text-sm font-extrabold text-[#536c89] sm:text-base">{job.company}</p></div><span className="shrink-0 rounded-lg bg-[#d5f6ee] px-3 py-1.5 text-xs font-black text-[#07876c]">{employmentTypeLabel(job.employmentType)}</span></div></section>
    ...
    <div className="fixed inset-x-0 bottom-0 z-40 border-t border-[#d9e8f7] bg-white/95 px-4 py-3 shadow-[0_-8px_26px_rgba(31,83,142,0.10)] backdrop-blur-xl"><div className="mx-auto flex max-w-3xl gap-3"><button type="button" onClick={() => { const next = toggleSaved(job.id); toast('success', next ? 'تم حفظ الوظيفة' : 'تمت إزالة الوظيفة من المحفوظات'); }} className="flex min-w-0 flex-1 items-center justify-center gap-2 rounded-[16px] border-2 border-[var(--accent-primary)] px-3 py-3 font-black text-[var(--accent-primary)]"><Bookmark className={`h-5 w-5 ${saved ? 'fill-current' : ''}`} /><span>{saved ? 'محفوظة' : 'حفظ الوظيفة'}</span></button><button type="button" onClick={() => setApplicationOpen(true)} className="flex-[1.7] rounded-[16px] bg-[var(--accent-primary)] px-4 py-3 font-black text-white shadow-[0_8px_22px_rgba(8,124,255,0.28)]">التقديم الآن</button></div></div>
